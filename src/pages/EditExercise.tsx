...
import { getExerciseById, updateExercise } from "../utils/exercises";
...
export function EditExercise() {
  ...
  useEffect(() => {
    const excercise = getExerciseById(id ?? "");
    if (excercise) {
      setName(excercise.name);
      setInstruction(excercise.instruction);
      setDifficulty(excercise.difficulty);
      setGoal(excercise.goalType);
      setEquipment(excercise.equipment);
      setAttributes(excercise.attributes);
      setImages(excercise.images);
      setVideos(excercise.videos);
    } else {
      navigate("/");
    }
  }, []);
